...
public class Main {
    static final int MAX_N = 103; // max n
    static final int MAX_LEN = 53; // max str.len

    static String[] v = new String[MAX_N];
    static int[][] cnt = new int[MAX_N][MAX_LEN]; // cnt[i][j] = k일 때, i번 단어 속 j번 문자의 번호는 k. (풀이 참고)

    /** 비슷한 단어인가 */
    static boolean isValid(int i, int j, int l) {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++)
            v[i] = br.readLine();
        final int l = v[0].length(); // 단어의 길이 (l ≤ 50)

        for (int i = 0; i < n; i++) {
            int[] ch = new int['z' + 3]; // ch['a'] = k일 때, 문자 a의 번호는 k
            for (int pos = 0, k = 1; pos < l; pos++) {
                char c = v[i].charAt(pos);
