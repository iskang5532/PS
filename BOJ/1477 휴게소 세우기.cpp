 ...
 * 만약 i to i-1의 길이가 mid보다 긴 경우 분할이 가능하며, mid보다 훨씬 큰 값이어서 두 번 이상 가능한 경우 추가로 카운팅.
 * 카운팅의 개수가 m보다 큰 경우(= cnt > m) mid값이 작으므로 값을 키워줌.
 * 카운팅의 개수가 m보다 작은 경우 (= cnt < m) mid값이 크므로 값을 줄여줌.
 * 이 때, cnt == m이면서 mid의 최솟값을 구해야 하므로 [cnt < m]을 [cnt <= m]으로 바꿔줌
