...
/*
    ...
    데드라인 전날에 해결할 수 없다면, 해결할 수 있는 날이 나올 때까지 이전 날을 확인하거나 1일이 될 때까지 반복하여 찾는다.
    ...
    - 큐를 사용해서 WA 한 번, 유파를 사용하지 않아서 TLE 한 번
 */
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        auto [d_day, k] = v[i]; // 데드라인 d_day, 컵라면의 수 k
        d_day = uf.find_parent(d_day);

        if (d_day >= 1) // 해결이 가능한 경우
            ans += k, uf.union_parent(d_day, d_day - 1);
    }
