...
void flooded(vector<vector<char>> &board, int R, int C) // 물과 인접한 칸을 침수시킴
{
    auto ret = board;
...
int bfs(vector<vector<char>> &board, pii start, pii end)
{
    int dist[MAX][MAX];
    for (int i = 0; i < R; i++)
    ...
    while (!q.empty())
    {
        ...
        q.pop();
        if (dist[y][x] == cnt)
            flooded(board, R, C), cnt++; // 침수시키며, 침수 횟수 +1
...
int main()
{
    ...
    int ans = bfs(board, start, end); // BFS를 통해 비버의 이동 시간을 구함
