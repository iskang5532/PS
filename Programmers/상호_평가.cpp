/*
    상호 평가 (코딩테스트 연습 > 위클리 챌린지 > 2주차(상호 평가))
    시간 복잡도: ?
    문제: https://programmers.co.kr/learn/courses/30/lessons/83201 (21-11-20. 이유는 모르겠으나 문제가 없어짐.)
    풀이)
    - 입력으로 주어지는 scores의 col, row가 바뀌었다보니 헷갈렸으며, 결국 다시 바꾼 값을 저장할 변수를 만듬
    etc.) 다른 사람들의 풀이를 본 후 내 풀이를 보니 너무 어렵게 푼 기분이 든다.
    - 21-11-20) col, row가 바뀌었다고 했는데 사실 본인 스스로가 col, row의 개념을 잘못 이해하고 있었음.. 다시 풀어보고 싶은데 문제가 없어졌음.
 */

#include <string>
#include <vector>
#include <numeric>   // accumulate
#include <algorithm> // sort, equal_range, max_element, min_element
